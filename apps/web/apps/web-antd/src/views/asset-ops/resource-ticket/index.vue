...
const provisionForm=ref({config_id:undefined as number|undefined,image_id:'',flavor:'',details:''});
...
const typeLabel:Record<string,string>={cloud:'云资源',network:'网络策略'};
...
async function fetchData(){loading.value=true;try{const rows=await Promise.all([getResourceTicketList(),getCloudZoneList(),getCloudPlatformList(),getMachineRoomList(),getSecurityProductList(),getCloudProviderConfigList(),getBusinessResourceList()]);data.value=rows[0] as any;zones.value=rows[1] as any;platforms.value=rows[2] as any;rooms.value=rows[3] as any;securityProducts.value=rows[4] as any;providerConfigs.value=rows[5] as any;businessResources.value=rows[6] as any;}catch{message.error('加载失败');}finally{loading.value=false;}}
onMounted(fetchData);
      ...
      <a-modal v-model:open="createVisible" title="创建资源工单" @ok="handleCreate" width="820px">
        <a-form layout="vertical">
          <a-row :gutter="16">
            ...
            <a-col v-if="isExistingResourceTicket" :span="24"><a-form-item label="目标业务资源" required><a-select v-model:value="createForm.target_resource_id" show-search :options="businessResources.map(item=>({value:item.id,label:`${item.ecs_name} / ${item.ip_address||'无 IP'}`}))" /></a-form-item></a-col>
            ...
            <a-col :span="12"><a-form-item label="资源类型" required><a-select v-model:value="createForm.resource_type" @change="handleResourceTypeChange"><a-select-option value="cloud">云资源</a-select-option><a-select-option value="network">网络策略</a-select-option></a-select></a-form-item></a-col>
