...
impl RustScan {
        ...
        for port in common_ports {
            if let Ok(addrs) = format!("{}:{}", target, port).to_socket_addrs() {
                ...
                if let Some(addr) = socket_addr {
                    ...
                    if timeout(Duration::from_millis(100), TcpStream::connect(&addr)).await.is_ok() {
                        return true;
                    }
                }
            }
    ...
    async fn scan_ports_concurrent(&self, target: &str, ports: &[u16]) -> Vec<PortScanResult> {
        ...
        let scan_futures: Vec<_> = ports.iter().map(|&port| {
            let target = target.clone();
            async move {
                Self::check_port_static(&target, port, timeout, retries).await
            }
        }).collect();
    ...
    async fn check_port_static(target: &str, port: u16, timeout_duration: Duration, retries: u32) -> PortScanResult {
        let socket_addrs: Vec<SocketAddr> = match format!("{}:{}", target, port).to_socket_addrs() {
            Ok(addrs) => addrs.collect(),
            Err(_) => {
                return PortScanResult {
    ...
    pub fn get_common_ports(strategy: &str) -> Vec<u16> {
        match strategy {
            ...
            "TOP100" => vec![
                ...
                119, 135, 139, 143, 144, 179, 199, 389, 427, 443, 445, 465, 506, 514, 515,
                543, 548, 554, 587, 631, 646, 873, 990, 993, 995, 1025, 1026, 1027, 1028,
                1029, 1110, 1433, 1434, 1521, 1582, 1583, 1720, 1723, 1755, 1900, 2000, 2001,
                2049, 2121, 2717, 3000, 3128, 3306, 3389, 3986, 4899, 5000, 5009, 5051, 5060,
                5061, 5101, 5190, 5357, 5432, 5631, 5666, 5800, 5900, 6000, 6001, 6646, 7070,
            ...
            "TOP1000" => {
                ...
                ports.extend(vec![
                    109, 123, 137, 138, 161, 162, 264, 311, 365, 369, 415, 416, 417, 512,
                    513, 517, 518, 520, 524, 540, 545, 546, 548, 554, 560, 563, 587, 591,
                    593, 616, 617, 625, 631, 636, 639, 646, 648, 652, 654, 660, 666, 674,
                    691, 700, 705, 711, 714, 720, 749, 765, 783, 787, 800, 801, 808, 843,
                    873, 880, 888, 898, 900, 901, 902, 903, 911, 912, 981, 987, 990, 992,
                    993, 994, 995, 996, 997, 998, 999, 1000, 1001, 1002, 1007, 1009, 1010,
                    1011, 1021, 1022, 1023, 1024, 1025, 1026, 1027, 1028, 1029, 1030, 1031,
                    ...
                    1158, 1164, 1169, 1174, 1175, 1183, 1185, 1186, 1187, 1192, 1198, 1199,
                    1201, 1213, 1216, 1217, 1218, 1233, 1234, 1236, 1244, 1247, 1248, 1259,
                    1271, 1277, 1287, 1296, 1300, 1301, 1309, 1310, 1311, 1322, 1328, 1334,
                    1352, 1417, 1433, 1434, 1443, 1455, 1461, 1494, 1500, 1501, 1503, 1521,
                    1524, 1533, 1556, 1580, 1583, 1594, 1600, 1641, 1658, 1666, 1687, 1688,
                    1700, 1717, 1718, 1719, 1720, 1721, 1723, 1755, 1761, 1782, 1783, 1801,
                    1805, 1812, 1813, 1839, 1840, 1862, 1863, 1864, 1875, 1900, 1914, 1935,
                    ...
                    6025, 6059, 6100, 6101, 6106, 6112, 6123, 6129, 6156, 6346, 6389, 6502,
                    ...
                    8899, 8900, 8910, 8994, 9000, 9001, 9002, 9003, 9009, 9010, 9011, 9040,
