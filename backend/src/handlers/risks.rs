...
pub async fn get_risks(State(state): State<AppState>, headers: HeaderMap) -> Result<Json<Vec<Risk>>, ApiError> {
...
pub async fn update_risk_status(State(state): State<AppState>, headers: HeaderMap, Path((id, status_str)): Path<(String, String)>) -> Result<Json<String>, ApiError> {
    ...
    let (found, updated_risk) = {
        let mut risks = state.risks.write()
...
pub async fn resolve_risk(State(state): State<AppState>, headers: HeaderMap, Path(id): Path<String>) -> Result<Json<String>, ApiError> {
    let user = get_current_user(&headers, &state.users)
        .ok_or_else(|| ApiError::unauthorized("Unauthorized"))?;
