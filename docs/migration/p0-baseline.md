...
- `0001_initial.sql`（11MB 合并基线）：public 48 表（system_* 37、infra_* 10、yudao_demo 3）+ 分 schema：ai 13、toonflow 26、toon 3、media 1。
...
| system | 86 | 保留 |
| ai | 62 | 保留 |
| bpm | 48 | yudao 工作流遗留，后端无对应模块 |
| infra | 42 | 基础设施，保留 |
| bid | 25 | yudao 招投标遗留，后端无对应模块 |
| toon | 18 | Toonflow 遗留 |

bpm/bid/toon 共 91 个权限码在菜单基线里但无后端实现，P3 菜单对齐时清理。
