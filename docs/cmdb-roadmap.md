...
- 这些保证覆盖上述应用写入路径，直接 SQL 写入必须遵守同一协议。当前按模型串行写入；后续高吞吐场景应使用数据库唯一约束缩小锁粒度。既有重复数据不会自动删除，租户归属保持现状。

验证：`cargo test -p rustset-cmdb-api -p rustset-cmdb-server -p rustset-infra-server`。设置 `TEST_DATABASE_URL` 后加 `-- --include-ignored`，在独立临时 schema 验证并发查重、补丁合并、工单回写和失败回滚。
