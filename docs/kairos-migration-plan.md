...
状态：P0、P1、P2、P3 以及 P4 资产运营首批迁移已于 2026-09-24 完成；P5 生产切流与 P6 稳定期收尾尚未实施。调研日期：2026-09-24。

当前完成摘要：infra 全部 223 条路由经集中注册表强制认证和权限码校验（未注册默认拒绝，能力/监控/上传等 5 条明确为仅登录）；bootstrap 管理员机制支持空密码条件更新与无管理员时的并发安全恢复创建；资产域新增 13 个动态菜单和 55 个按钮权限。13 个 Kairos/Vben 资产页面已通过集中兼容层接入本地 `/infra/*` API，按钮使用同一权限码；机柜、扫描 Agent、云厂商自动开通等本地尚无真实能力的功能未接假接口。新增迁移均通过空库与重入测试，参考快照已刷新。P1 摘要：全仓 CRLF 归一化；bun 正式工具链；上游菜单转换修正；RustSet 品牌；Dioxus 已删除。交付：`docs/migration/p0-baseline.md`、`docs/migration/upstream-diff.md`。
